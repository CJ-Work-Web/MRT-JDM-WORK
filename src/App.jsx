...
/**
 * 安全配置與環境適應性加固：
 * 考慮到不同部屬環境對 ES 版本及環境變數讀取語法的支援差異，
 * 我們使用更具相容性的方式來讀取 Firebase 設定與 App ID。
 */
const getFirebaseConfig = () => {
  // 1. 優先從 Canvas 系統注入的變數讀取
  if (typeof __firebase_config !== 'undefined' && __firebase_config) {
    return JSON.parse(__firebase_config);
  }
  
  // 2. 嘗試從 process.env 讀取 (適用於 Vercel / Webpack / CRA 等環境)
...
const getAppId = () => {
  ...
  // 2. 嘗試從 process.env 讀取
  if (typeof process !== 'undefined' && process.env && process.env.VITE_APP_ID) {
...
function getInitialFormState() {
  return {
    ...
    costItems: [{ id: crypto.randomUUID(), contractor: '', workTask: '', invoiceNumber: '', billingDate: '', costAmount: '', voucherNumber: '', remarks: '' }],
    incomeItems: [{ id: crypto.randomUUID(), source: '晟晁', receiptNumber: '', receiveDate: '', subtotal: 0, serviceFee: 0, tax: 0, incomeAmount: 0, incomeVoucherNumber: '', remarks: '' }],
...
const App = () => {
  ...
  useEffect(() => {
    if (!user || !hasActivatedDashboard) return;
          ...
          <>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="flex items-center gap-4 shrink-0">
                ...
                <div><h1 className="text-xl font-black text-slate-900 tracking-tight">{currentDocId ? '編輯現有案件' : '建立新維修單'}</h1><div className="flex items-center gap-2 mt-1"><span className={`w-2.5 h-2.5 rounded-full ${user ? 'bg-emerald-500 animate-pulse' : 'bg-orange-400 animate-bounce'}`}></span><span className="text-xs font-black text-slate-500 uppercase tracking-widest">{!user ? '正在連線中...' : currentDocId ? `案件 ID: ${currentDocId.slice(0,8)}` : '雲端連線成功'}</span></div></div>
              </div>
              <div className="flex-1 flex flex-wrap justify-end items-center gap-3 w-full">
                ...
                <div className="flex items-center gap-3 ml-2 border-l-2 pl-3 self-start">
                  ...
                  <button onClick={handleSaveToCloud} disabled={isSaving} className={`flex items-center gap-2 px-8 py-2 text-white rounded-xl font-black text-sm transition shadow-lg active:scale-95 whitespace-nowrap shrink-0 ${currentDocId ? 'bg-purple-600 hover:bg-purple-700' : 'bg-blue-600 hover:bg-blue-700'}`}>{isSaving ? <Clock className="animate-spin" size={16} /> : <Save size={16} />} {isSaving ? '儲存中' : '儲存案件'}</button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200">
              ...
              <div className="flex justify-end mt-4 pt-6 bg-slate-50/50 p-6 rounded-2xl border border-slate-100 shadow-inner"><div className="space-y-3 w-64 text-right"><div className="flex justify-between text-sm font-black text-slate-600 uppercase tracking-tight whitespace-nowrap"><span className="shrink-0 mr-4">小計 (未稅)</span><span className="text-slate-700 font-mono font-black">${calculationSummary.subtotal.toLocaleString()}</span></div>{formData.repairType === '2.1' && (<div className="flex justify-between text-sm font-black text-slate-600 uppercase tracking-tight whitespace-nowrap"><span className="shrink-0 mr-4">服務費 (5%)</span><span className="font-mono font-black">${calculationSummary.serviceFee.toLocaleString()}</span></div>)}<div className="flex justify-between text-sm font-black text-slate-600 uppercase tracking-tight whitespace-nowrap"><span className="shrink-0 mr-4">稅金 (5%)</span><span className="text-slate-700 font-mono font-black">${calculationSummary.tax.toLocaleString()}</span></div><div className="h-px bg-slate-200 my-2"></div><div className="flex justify-between items-center text-2xl font-black text-blue-700 font-mono tracking-tighter whitespace-nowrap"><span className="text-xs text-blue-800 uppercase tracking-widest shrink-0 mr-4">總價(稅後)</span>${calculationSummary.final.toLocaleString()}</div></div></div>
            </div>
