  ...
  return (
    <tr className="hover:bg-slate-50/50 group transition-colors border-b last:border-none border-slate-100 text-[11px] md:text-xs">
      <td className="p-2 text-center">
        <span className={`px-2 py-0.5 rounded-full font-black inline-flex justify-center shadow-sm whitespace-nowrap w-14 ${!status ? 'bg-slate-300 text-slate-700' : status === '結報' ? 'bg-emerald-100 text-emerald-700' : status === '退件' ? 'bg-rose-100 text-rose-700' : 'bg-blue-100 text-blue-700'}`}>
          {status || '待提報'}
        </span>
      </td>
      <td className="p-2 text-center">
      ...
      <td className="p-2">
        <div className="text-slate-900 font-black break-words">{String(item.jdmControl?.caseNumber || '未編號')}</div>
        <div className="font-bold text-[9px] text-slate-500 mt-0.5 uppercase break-words">{String(item.station || '未知')}</div>
      </td>
      <td className="p-2">
        <div className="font-black text-slate-900 break-words">{String(item.tenant || '--')}</div>
        <div className="text-[9px] text-slate-500 mt-0.5 break-words leading-relaxed font-bold" title={String(item.address || '')}>{String(item.address || '無地址')}</div>
      </td>
      <td className="p-2">
        <div className="font-bold text-slate-600 break-words leading-relaxed" title={String(item.quoteTitle || '')}>
...
const App = () => {
  ...
  const handleDeleteTrigger = useCallback((id) => { setPendingDeleteId(id); setIsDeleteModalOpen(true); }, []);
        ...
        const handleResetDashboardFilters = () => {
          setDashboardFilter({
            search: '',
            status: '未完成案件', // Default status
            stations: [],
            reportMonth: '',
            closeMonth: '',
            specialFormula: '',
            baseMonth: new Date().toLocaleDateString('en-CA').slice(0, 7) // Recalculate current month
          });
          setIsAllTimeSearch(false); // Reset this as well, it's related to search.
          setIsSpecialSearchOpen(false); // Close the special search dropdown
          setStationSearch(''); // Clear station search input
          showMessage("已重置所有篩選條件", "success"); // Provide feedback
        };
...
<option>未完成案件</option><option>全部</option><option disabled className="bg-slate-100 text-slate-400">───── 常規狀態 ─────</option><option>待提報</option><option>提報</option><option>抽換</option><option>退件</option><option>結報</option></select></div>
                  ...
                  </div> {/* Added closing relative container */}
                  </div>
                  <button onClick={handleResetDashboardFilters} className="flex items-center gap-2 px-4 py-3 rounded-2xl font-black text-sm border transition-all hover:bg-slate-50 shadow-sm bg-slate-50 border-slate-300 text-slate-700">
